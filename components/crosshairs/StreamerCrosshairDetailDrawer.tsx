...
export function StreamerCrosshairDetailDrawer({
  profile,
  version,
  onClose
}: {
  ...
  const analysisEntries = version.analysis ? Object.entries({
    "尺寸": version.analysis.size,
    "遮挡": version.analysis.occlusion,
    "背景可见度": version.analysis.backgroundVisibility,
    "远距离": version.analysis.longRange,
    "近距离": version.analysis.closeRange,
    "单点": version.analysis.tapFire,
    "短连发": version.analysis.burstFire,
    "扫射": version.analysis.spray,
    "新手适应": version.analysis.beginnerAdaptation,
    "瞄准习惯": version.analysis.aimingHabit
  }) : [];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/78" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="streamer-detail-title"
      ...
      >
        <header className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-white/[0.08] bg-[#0c0d0f]/96 px-5 py-4 backdrop-blur-md sm:px-7">
          <div className="min-w-0">
            ...
            <h2 id="streamer-detail-title" className="truncate text-lg font-semibold text-white">{profile.displayName} · {version.titleZh}</h2>
          </div>
          <button ref={closeRef} type="button" onClick={onClose} aria-label="关闭准星详情" className="grid h-11 w-11 shrink-0 place-items-center rounded-md border border-white/15 text-xl text-zinc-400 transition hover:border-white/30 hover:text-white focus-visible:outline focus-visible:outline-1 focus-visible:outline-valorant/70">×</button>
        </header>
        ...
        <div className="space-y-7 px-5 py-6 sm:px-7 sm:py-8">
          <section className="flex flex-wrap items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-semibold text-white">{profile.displayName}{profile.realName ? ` · ${profile.realName}` : ""}</p>
              <p className="text-xs text-zinc-500">{profile.country} · {profile.streamLanguage} · {profile.ladderRegion}</p>
            </div>
            <VerificationBadge status={version.verificationStatus} />
          </section>

          <a href={profile.channelUrl} target="_blank" rel="noopener noreferrer" className="btn-secondary min-h-11 px-3">打开 {profile.channelName} 频道 ↗</a>
          ...
          <section className="crosshair-code-panel p-4">
            ...
            <div className="mt-4"><CopyButton value={version.code} disabled={!validation.valid} invalidReason={validation.reasons.join("；")} /></div>
          </section>

          <section className="drawer-section">
            <h3 className="text-lg font-semibold text-white">准星参数</h3>
            <SettingsTable settings={version.normalizedSettings} />
          </section>

          {analysisEntries.length ? (
            <section className="drawer-section">
              <h3 className="text-lg font-semibold text-white">ClutchNest 原创分析</h3>
              <dl className="mt-4 grid gap-x-6 sm:grid-cols-2">
                {analysisEntries.map(([label, value]) => (
                  <div key={label} className="border-t border-white/[0.08] py-3">
          ...
          <div className="drawer-section grid gap-7 sm:grid-cols-3">
          ...
          <section className="drawer-section">
            <div className="border-l border-valorant/80 pl-4">
              <h3 className="text-sm font-semibold text-white">HAO 测试状态</h3>
              <p className="mt-2 text-sm leading-6 text-zinc-400">尚未实战测试。当前内容基于公开代码、参数和视觉特征分析，不代表 HAO 的实战结论。</p>
            </div>
          </section>

          <section className="drawer-section">
            <h3 className="text-sm font-semibold text-white">活动、天梯与设置来源</h3>
            <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
              <Meta label="公开段位" value={profile.publicRank ?? "未记录"} />
              <Meta label="天梯状态" value={ladderStatusLabel(profile.ladderStatus)} />
              <Meta label="天梯核验" value={profile.rankVerifiedAt} />
              <Meta label="最近内容" value={profile.lastValorantContentAt ?? "未记录"} />
            </dl>
            <div className="mt-5 grid">
              {[...profile.rankSources, ...profile.activitySources, ...version.sources].map((source) => (
                <a key={`${source.url}-${source.title}`} href={source.url} target="_blank" rel="noopener noreferrer" className="border-t border-white/[0.08] px-1 py-3 text-sm text-zinc-400 transition hover:text-white focus-visible:outline focus-visible:outline-1 focus-visible:outline-valorant/70">
                  <span className="block font-medium">{source.title}</span>
                  <span className="mt-1 block text-xs text-zinc-600">访问 {source.accessedAt}{source.publishedAt ? ` · 发布 ${source.publishedAt}` : ""}</span>
                </a>
              ))}
            </div>
          </section>

          <p className="drawer-section text-xs leading-6 text-zinc-600">公开设置会随时间变化。使用相同准星不会带来相同竞技表现；请根据自己的分辨率、视觉习惯和射击方式调整。</p>
        </div>
      </div>
    </div>
  );
...
function SettingsTable({ settings }: { settings?: NormalizedCrosshairSettings }) {
  const rows = settings ? [
    ["颜色", [settings.colorName, settings.colorHex].filter(Boolean).join(" · ")],
    ["描边", yesNo(settings.outlinesEnabled)],
    ["中心点", yesNo(settings.centerDotEnabled)],
    ["内线", yesNo(settings.innerLinesEnabled)],
    ["内线长度 / 厚度 / 偏移", [settings.innerLineLength, settings.innerLineThickness, settings.innerLineOffset].map(numberOrUnknown).join(" / ")],
    ["外线", yesNo(settings.outerLinesEnabled)],
    ["移动误差 / 射击误差", `${yesNo(settings.innerMovementError)} / ${yesNo(settings.innerFiringError)}`]
  ] : [["参数", "未公开"]];
