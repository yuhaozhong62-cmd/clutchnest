    ...
    <>
          ...
          <div className="mt-8 divide-y divide-white/[0.08] border-y border-white/[0.08]">
            {latestUpdates.map((item) => (
              <Link key={item.id} href={item.href} className="group grid gap-2 py-5 transition hover:bg-white/[0.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:px-3">
                <div className="min-w-0"><p className="font-semibold text-white">{item.title}</p><p className="mt-1 line-clamp-1 text-sm text-zinc-400">{item.description}</p></div>
                <div className="flex items-center gap-4 text-xs text-zinc-500"><span>{item.updatedAt ? formatDateCn(item.updatedAt) : "更新时间未记录"}</span><span className="transition group-hover:translate-x-0.5 group-hover:text-white" aria-hidden="true">→</span></div>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="section-shell bg-white/[0.012]">
        <div className="site-container">
          ...
          <Link href="/crosshairs?type=streamer" className="surface-card group mt-8 grid gap-4 p-5 focus-visible:outline-none sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
...
function FeaturedTeam({ team }: { team: CrosshairTeamDefinition }) {
  ...
  return (
    <Link href={`/crosshairs?team=${team.id}`} className="surface-card group p-5 focus-visible:outline-none">
      <div className="flex items-center justify-between gap-3"><span className="grid h-10 w-10 place-items-center bg-white text-xs font-black text-black">{team.shortName}</span><span className="text-xs text-zinc-500">{team.fullName}</span></div>
...
function SectionHeading({ title, subtitle, href, linkLabel }: { title: string; subtitle: string; href: string; linkLabel: string }) {
  return <div className="flex items-end justify-between gap-5"><div><h2 className="section-title">{title}</h2><p className="mt-2 support-copy">{subtitle}</p></div><Link href={href} className="shrink-0 text-sm font-semibold text-zinc-300 transition hover:text-white focus-visible:outline-none">{linkLabel} <span aria-hidden="true">→</span></Link></div>;
}
